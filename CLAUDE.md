...
- **Models** — value types describing the inputs:
  ...
  - `DocumentationConfiguration` — capture + generation options. Supplied at `DocumentedFlow.init` (not just at `generateDocumentation`) because the comparison **tolerances are consumed at capture time**: `snapshotPrecision`/`snapshotPerceptualPrecision` map `overallTolerance`/`perPixelTolerance` (as `1 - tolerance`, clamped) onto swift-snapshot-testing's `.image(precision:perceptualPrecision:)`. The remaining flags (`deviceFrames`, `includeFlowDiagram`, `organizeByDevice`, `createIndexPage`) are declared but not yet wired into the generator.
