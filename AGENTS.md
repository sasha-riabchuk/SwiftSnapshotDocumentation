...
| A **Liquid Glass** (`.glassEffect()`) button/screen is transparent or blank | Glass samples the backdrop; the default offscreen render skips backdrop filters | Try `captureMode: .hostWindow` for real-compositor capture (host-app test target only); otherwise substitute a **solid fill** via your own style/env flag in your code |
