...
`HostApp/` is a separate, generated Xcode project (not part of the SPM package) that captures **pixel-exact** Liquid Glass — the only faithful path, since no in-process snapshot composites backdrop effects. It's a minimal app that presents a glass screen by `-glassScreen`/`-appearance` launch args + a **UI-test target** that reads the real framebuffer (`XCUIScreen.screenshot()`) and writes PNGs to `Sources/UITests/__Framebuffers__/`. `generate.rb` (Ruby `xcodeproj` gem) regenerates `GlassProof.xcodeproj`. A throwaway SPM test can build the glass Flow Explorer from those PNGs via `exportFlowExplorer(snapshotSourcePath:)`. Run the UI test on a **real device** for device-accurate glass (the Simulator only approximates it).
